...
class TestViews(TestCase):
	...
	def test_can_register_user(self):
		response=self.client.post(self.register_url,format='/user/register.html')
		self.assertEqual(response.status_code,200)

	"""def test_can_user_profile(self):
					response=self.client.post(self.profile_url,format='/user/profile.html')
